...
import { _decorator, Node, ScrollView, EventHandler, Widget, Vec3, EventTouch, math, v2, v3, UITransform } from 'cc';
...
export interface UISuperHeaderAndFooterEvent {
     ...
     * release=释放中 
     */
    progressStage: string //"touch" | "wait" | "lock" | "release",
}

@ccclass('UISpuerScrollView')
export class UISpuerScrollView extends ScrollView {
    @property({
        displayName: "顶部偏移量",
    ...
    @property({
        type: EventHandler,
        displayName: "下拉事件"
    }) pullDownEvents: EventHandler[] = []
    @property({
        type: EventHandler,
        displayName: "上拉事件"
    }) pullUpEvents: EventHandler[] = []
    ...
    private _layout: UISuperLayout | null = null
    private get layout(): UISuperLayout {
        if (this._layout == null) this._layout = this.content?.getComponent(UISuperLayout) ?? new UISuperLayout();
        return this._layout
    }
    ...
    private get isHeader() {
        if (this.layout.headerToFooter) {
            if (this.layout?.header) {
                return this.layout?.header.getSiblingIndex() == 0
            }
        } else {
            if (this.layout?.footer) {
                return this.layout?.footer.getSiblingIndex() == this.layout.maxItemTotal - 1
            }
        }
        return true
    }
    ...
    public getHowMuchOutOfBoundary(offset?: Vec3) {
        return this._getHowMuchOutOfBoundary(offset)
    }
    onLoad() {
        this.node.on(Node.EventType.SIZE_CHANGED, this.onChangeSize, this)
    }
    onDestroy() {
        this.node.off(Node.EventType.SIZE_CHANGED, this.onChangeSize, this)
    }
    private onChangeSize() {
        let widget = this.view?.getComponent(Widget)
        if (!widget) return
    ...
    public reset() {
        ...
        if (!offset.equals(v3(0, 0, 0), EPSILON)) {
            this._processInertiaScroll()
        }
    }
    ...
    protected _getContentTopBoundary() {
        let viewSize = this.view?.contentSize ?? new math.Size();
        let local = 0
    ...
    protected _getContentBottomBoundary() {
        let viewSize = this.view?.contentSize ?? new math.Size();
        let uit = this.layout.node.getComponent(UITransform) ?? new UITransform();
        let local = 0
        if (this.layout?.footer && this.layout.getReallySize().height > viewSize.height) {
            local = this.layout.bottomBoundary //返回尾部item上边距
        } else {
            //功能用于无内容/少量内容时也可以上拉加载下拉刷新 如果所有item加起来的尺寸不足以撑满整个可视区域时 直接使用view可视尺寸
            local = this.layout.node.getPosition().y - uit.anchorY ?? 0 * viewSize.height;
        }
    ...
    protected _getContentLeftBoundary() {
        ...
        if (this.layout?.header && this.layout.getReallySize().width > viewSize.width) {
            ...
            //功能用于无内容/少量内容时也可以上拉加载下拉刷新 如果所有item加起来的尺寸不足以撑满整个可视区域时 直接使用view可视尺寸
            local = this.layout.node.getPosition().x - uit.anchorX * viewSize.width;
        }
        if (this.isHeader && this.isLockHeader) {
            local -= this.headerOutOffset  //功能用于上拉加载 下拉刷新 让整个content多一个 headerOutOffset 的尺寸
        }
    ...
    protected _startAutoScroll(deltaMove: Vec3, timeInSecond: number, attenuated?: boolean) {
        if (this.isCalculPull) {  // 如果没有刷新/加载的监听者 则不计算 
            if (this.isMoveHeader && !this.isLockHeader) { // 锁住头部 意思就是已经触发了下拉事件 应用层应该做些刷新的动作
                ...
                this.vertical && (deltaMove.set(deltaMove.x, deltaMove.y - this.headerOutOffset))
                this.horizontal && (deltaMove.set(deltaMove.x + this.headerOutOffset))
                ...
                this.vertical && (deltaMove.set(deltaMove.x, deltaMove.y + this.footerOutOffset))
                this.horizontal && (deltaMove.set(deltaMove.x - this.footerOutOffset))
    ...
    protected _updateScrollBar(outOfBoundary: math.Vec2) {
    ...
    private emitPullDownEvent(data: UISuperHeaderAndFooterEvent) {
        EventHandler.emitEvents(this.pullDownEvents, this, data)
    }
    private emitPullUpEvent(data: UISuperHeaderAndFooterEvent) {
        EventHandler.emitEvents(this.pullUpEvents, this, data)
    }
}